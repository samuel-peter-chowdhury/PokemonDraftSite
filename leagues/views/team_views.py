...
def team_standings_view(request, id):
    if request.user.has_league(id):
        ...
        teamStandings = sorted(teamStandings, key = lambda x: (-x['match_win_percentage'], -(x['match_win_count'] - x['match_loss_count']), -x['game_win_percentage'], -(x['game_win_count'] - x['game_loss_count']), -x['pokemon_differential']))
